...
def parse_obj_file(file_path):
    ...
    return jax.numpy.array(vertices), jax.numpy.array(texture_coords), jax.numpy.array(faces)


def render_triangles(vertices, texture_coords, faces, texture, width, height):
    image = jax.numpy.zeros((height, width, 3), dtype=jax.numpy.uint8)
    depth_buffer = jax.numpy.full((height, width), float('-inf'))
    ...
    for face in faces:
        ...
        x, y = jax.numpy.meshgrid(jax.numpy.arange(max(min_x, 0), min(max_x + 1, width)),
                                  jax.numpy.arange(max(min_y, 0), min(max_y + 1, height)))
        ...
        # Update depth buffer and image for pixels inside the triangle using JAX .at method
        update_mask = mask & (depth > depth_buffer[y, x])
        depth_buffer = depth_buffer.at[y[update_mask], x[update_mask]].set(depth[update_mask])
        color = texture[jax.numpy.clip(ty[update_mask] * texture.shape[0], 0, texture.shape[0] - 1).astype(int),
                        jax.numpy.clip(tx[update_mask] * texture.shape[1], 0, texture.shape[1] - 1).astype(int)]
        image = image.at[y[update_mask], x[update_mask]].set(color)
...
def main():
    ...
    texture = jax.numpy.array(Image.open('african_head_diffuse.tga'))

    image = render_triangles(vertices, texture_coords, faces, texture, 32, 32)
