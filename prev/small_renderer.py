    ...
    with open(file_path, 'r') as file:
        for line in file:
            if line.startswith('v '):
                ...
                faces.append([tuple(int(idx) - 1 if idx else None for idx in vert.split('/')) for vert in line.split()[1:]])
...
def render_triangles(vertices, texture_coords, faces, texture, width, height):
    ...
    for face in faces:
        for i in range(1, len(face) - 1):
            v0, v1, v2 = [((vertices[face[j][0]][0] + 1) * width / 2, (1 - vertices[face[j][0]][1]) * height / 2, vertices[face[j][0]][2]) for j in [0, i, i + 1]]
            vt0, vt1, vt2 = [texture_coords[face[j][1]] for j in [0, i, i + 1] if face[j][1] is not None]
