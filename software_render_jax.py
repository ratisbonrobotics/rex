...
def rasterize_pixel(x, y, v0, v1, v2, vt0, vt1, vt2, texture, depth_buffer):
    ...
    def true_fn(args):
        ...
        # Perform depth test
        depth_buffer = jax.lax.cond(depth > depth_buffer[x, y],
                                    lambda _: depth_buffer.at[x, y].set(depth),
                                    lambda _: depth_buffer,
                                    None)
        ...
        color = texture[jnp.floor(ty * texture.shape[0]).astype(int),
                        jnp.floor(tx * texture.shape[1]).astype(int)]
        return color.astype(jnp.uint8), depth_buffer

    def false_fn(args):
        depth_buffer = args[3]
        return jnp.array([0, 0, 0], dtype=jnp.uint8), depth_buffer

    return jax.lax.cond(jnp.logical_and(jnp.logical_and(w0 >= 0, w1 >= 0), w2 >= 0),
                        true_fn,
                        false_fn,
                        (w0, w1, w2, depth_buffer))
...
def render_triangles(vertices, texture_coords, faces, texture, width, height):
    ...
    rasterize_pixel_jit = jax.jit(rasterize_pixel)
    ...
    for face in faces:
        ...
        for y in range(min_y, max_y + 1):
            for x in range(min_x, max_x + 1):
                color, depth_buffer = rasterize_pixel_jit(x, y, v0, v1, v2, vt0, vt1, vt2, texture, depth_buffer)
